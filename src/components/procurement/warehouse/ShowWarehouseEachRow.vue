<template>
  ...
  ' bg-white  hover:bg-indigo-500 '"
      ...
      <TableRow :each = "prop?.each" :table_headers="procurement_store.GETWAREHOUSEHEADERS" />
...
const checkedRows = () => {
  ...
  if (checked_style.value) {
      procurement_store.warehouse_selecting_rows.push(prop.each)
  }
  else {
      procurement_store.warehouse_selecting_rows = procurement_store.warehouse_selecting_rows.filter((item) => item.stf_id !== prop.each.stf_id)
  }
}
